...
import { VirtualPrismaRepository } from '@src/framework/clean-architecture/infrastructure/repositories/prisma/generate-virtual-prisma-repository-reference';
 ...
 */
export type EntityProperties<T> = OmitFunctions<
...
export function generatePrismaCrudRepository<
  ...
  EntityVirtualPrismaRepository extends VirtualPrismaRepository<
    PrismaEntityType,
    Repository
  >,
...
>({
  ...
  abstract class PrismaCrudRepository {
    ...
    constructor(prismaService: PrismaService) {
      this.internalPrismaRepository = prismaService[
        entityVirtualPrismaRepository.name as any
      ] as any;
    ...
    protected ModelInformationClass: ClassType<CreateModelInformation> =
      CreateModelInformationClass;

    abstract entityToModel(entity: PrismaEntityFields): Model | Promise<Model>;

    abstract modelToEntity(
      model: Model,
    ): PrismaEntityFields | Promise<PrismaEntityFields>;
    ...
    abstract createModelInformationToEntity(
      ...
      | Promise<CreationPrismaEntityFields<PrismaEntityFields>>;
    ...
    public async create(
      ...
      const parsedInformation = await this.createModelInformationToEntity(
        validatedModelInformation,
      );
    ...
    public async saveModel(model: Model): Promise<Model> {
      const validatedModel = await transformUnknownAndValidate(
        ModelClass,
        model,
      );
